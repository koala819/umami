...
async function applyMigration() {
  if (!process.env.SKIP_DB_MIGRATION) {
    ...
    console.log(`Applying migrations using URL: ${connectionUrl.split('@')[1] || 'local'}`); // Masquer les identifiants
    console.log(execSync(`DATABASE_URL="${connectionUrl}" prisma migrate deploy`).toString());
